        ...
        for idx, line in enumerate(f):
            ...
            if line != "":
                ...
                for cell_idx, cell in enumerate(line):
                    cube[center_idx][idx+simulation_length+1][cell_idx+simulation_length+1] = 1 if cell == ACTIVE else 0
    return cube
    ...
    for idx0 in range(1, shape-1):
        for idx1 in range(1, shape-1):
            for idx2 in range(1, shape-1):
                new_cube[idx0][idx1][idx2] = calculate_new_value(cube, (idx0, idx1, idx2))
    return new_cube
    ...
    for idx0 in range(shape):
        for idx1 in range(shape):
            for idx2 in range(shape):
                if cube[idx0][idx1][idx2] == 1:
                    number_of_active_cells += 1
    return number_of_active_cells

def zero_cube(shape):
    cube = [[[0] * shape for _ in range(shape)] for __ in range(shape)]
    return cube

def get_neighbors(cube, multi_index):
    idx0, idx1, idx2 = multi_index
    result = []
    for offset0, offset1, offset2 in product([-1, 0, 1], repeat=3):
        if not offset0 == offset1 == offset2 == 0:
            result.append(cube[idx0 + offset0][idx1 + offset1][idx2 + offset2])
    return result

def calculate_new_value(cube, multi_index):
    neighbors = get_neighbors(cube, multi_index)
    idx0, idx1, idx2 = multi_index
    curr_value = cube[idx0][idx1][idx2]
...
if __name__ == "__main__":
    print(solve_task1("day_17/input.txt"))
